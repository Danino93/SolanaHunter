-- ============================================================================
-- טבלאות Smart Wallets - גרסה בטוחה (לא כושלת אם כבר קיים)
-- ============================================================================
-- SQL מתוקן שעובד גם אם חלק מהדברים כבר קיימים
-- ============================================================================

-- ============================================================================
-- 1. טבלת Smart Wallets (רק אם לא קיימת)
-- ============================================================================
...
-- ============================================================================
-- 2. טבלת היסטוריית טוכנים (רק אם לא קיימת)
-- ============================================================================
...
-- ============================================================================
-- 4. יצירת Indexes (רק אם לא קיימים) - בטוח
-- ============================================================================

-- Indexes for smart_wallets (בדיקה אם לא קיימים)
DO $$
BEGIN
...
-- Indexes for scanned_tokens_history (בדיקה אם לא קיימים)
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_scanned_tokens_smart_money_count') THEN
        CREATE INDEX idx_scanned_tokens_smart_money_count ON scanned_tokens_history(smart_money_count);
    END IF;
END $$;

-- Indexes for wallet_token_holdings (בדיקה אם לא קיימים)
DO $$
BEGIN
...
-- ============================================================================
-- 5. פונקציות (להחליף אם קיימות)
-- ============================================================================

-- פונקציה לעדכון updated_at (החלף אם קיימת)
...
-- פונקציה להוספת Smart Wallet (החלף אם קיימת)
...
-- פונקציה לעדכון Trust Score (החלף אם קיימת)
...
-- פונקציה לרישום Token חדש (החלף אם קיימת)
...
-- ============================================================================
-- 6. הוספת Foreign Keys (בטוח)
-- ============================================================================
DO $$
BEGIN
    -- בדוק אם הקשר כבר קיים לפני הוספה
    IF NOT EXISTS (
...
-- ============================================================================
-- 8. בדיקה סופית - מה נוצר בהצלחה
-- ============================================================================
SELECT 'Smart Wallets system setup completed!' as result;

-- בדיקת טבלאות שנוצרו
SELECT 
    'Tables created: ' || string_agg(table_name, ', ') as tables_created
FROM information_schema.tables 
WHERE table_schema = 'public' 
    AND table_name IN ('smart_wallets', 'scanned_tokens_history', 'wallet_token_holdings');

-- בדיקת פונקציות שנוצרו  
SELECT 
    'Functions created: ' || string_agg(routine_name, ', ') as functions_created
FROM information_schema.routines
WHERE routine_schema = 'public'
    AND routine_name IN ('add_smart_wallet', 'update_trust_score', 'log_scanned_token');